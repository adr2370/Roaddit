...
function getPlacesAlongRoute($x0,$y0,$x1,$y1,$dir,$types) {
	...
	if($cir==1) {
		...
		foreach($urls as $count => $url) {
			...
			$hotel = json_decode(curl_multi_getcontent($curl_array[$count+2]));
			$curr = $places[$count/3];
			array_push($curr,$res->results[0]);
			array_push($curr,$food->results[0]);
			array_push($curr,$hotel->results[0]);
			$places[$count/3]=$curr;
...
function getEverything($x0,$y0,$x1,$y1,$maxDur,$maxCost,$types,$startTime) {
	...
	if(count($trips)==0) {
		$places=0;
		$tripOutput="|Start|".$x0."|".$y0."|0|0|StartPic";
		$tripOutput.="|End|".$x1."|".$y1."|0|0|EndPic|";
